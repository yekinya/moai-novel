...
- [HARD] **마법명과 일반 마법 용어는 한국어로 통일**한다.
- 예시 매핑 (강제):
  - `라이트` → `광명`
  - `파이어볼` → `화염구`
  - `메테오` → `운석강하`
  ...
  - `디바인 파워` → `신성력`
- 예외 — **외래어/원어 표기 그대로 유지**:
  - 인물명: 라인하르츠 행커만, 일레인 맥필렌, 데미안 스펠른, 모티어스 매리앤, 아라곤 스틸만, 바이서스 카멜라 4세 등
  - 사건명·전투명: 칠흑성 전투, 검은평원 전투, 신좌침묵, 암흑대전 등
  - 국가·도시·지명: 헤븐즈 킹덤, 엘더스톰, 강철제국, 미드그레이드, 무스페름, 리펠리아 등
  - 종족명: 엘프, 드워프, 오크, 하플링 등 (이미 외래어로 굳어진 표준 표기)
  - 세계관 고유 용어: 르세프, 크세프, 세프, 셉터, 세프렌, 셉터렌, 스펠, 써클, 리펠리트, 다르켈, 엘라/엘두르/학카르 등 하위 법칙명
  - 무기·도구 표준명: 바스타드 소드, 모닝스타 등 (RPG 표준 무기로 굳어진 명칭)
- 첫 등장 시 영문 병기 1회는 허용 (예: `광명(Light)`).
- 새 마법명을 추가할 때도 항상 한국어 명칭을 먼저 만들고 필요 시 영문 병기.
- 기존 원고/설정에서 외래어로 쓰인 마법명을 발견하면 즉시 한국어로 치환한다.
